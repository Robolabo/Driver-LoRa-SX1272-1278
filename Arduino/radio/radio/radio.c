...
DEFINE_RADIO;								//Necesaria para definir la estructura RADIO
...
u1_t tiempoPorSimbolo ()					//Ts(ms)= 2^SF / (BW {hz}) * 1000
{
    ...
    Ts_Condicion  = 0;
    Ts = (((u4_t)1 << RADIO.sf) / BW);   				//Ts(ms)= 2^SF / (BW {hz}) * 1000
...
void writeBuf (u1_t addr, xref2u1_t buf, u1_t len)
{
	u1_t i;
	hal_pin_nss(0);					//Inicia uso de SPI NSS->0
    hal_spi(addr | 0x80);			//Escribe la direccion del BUFFER FIFO del LORA (por def es 0X00)
    for (i=0; i<len; i++)		//Escribe los datos de RADIO.frameTX[..] hasta alcanzar la  cantidad de datos RADIO.dataLenTX
    {
        hal_spi(buf[i]);
    }
...
void readBuf (u1_t addr, xref2u1_t buf, u1_t len)		//Utilizada para leer los datos del la FIFO. addr-> DIRECCION inicial a leer     buf->VECTOR de alamcenamiento  len-> longitud de los datos a leer
{
    ...
    for (i=0; i<len; i++)		//Lee los datos del FIFO y los transcribe a RADIO.frameRX[..] hasta alcanzar la  cantidad de datos RADIO.dataLenRX
    {
        buf[i] = hal_spi(0x00);
    }
    hal_pin_nss(1);					//EL SPI Finaliza con NSS<-1    Detiene el BUS SPI
...
void radio_hal_init ()										//Utiliza el protocolo de arranque del chip propuesto por Semtech
{
	...
	hal_delay(10);											//Hace un delay de 5ms recomendado por el fabricante

#if defined(CFG_1272_SEMTECH)                          //Selecciona el CHIP SX1272
	hal_pin_rst(1); 										//Realiza un reset en el MODEM RST->1, hace el reset manual (SX1272)
    hal_delay(10);											//Hace un delay de 100us (exigido por el fabricante
    hal_pin_rst(0);											//Coloca el pin de RESET del modem como FLOTANTE (pin que antes era salida, ahora es entrada)
#endif

#if (defined CFG_1278_DRF1278F)  ||  (defined CFG_1278_NICERF1278)    //Selecciona el CHIP SX1278
	hal_pin_rst(0); 										//Realiza un reset en el MODEM RST->0, hace el reset manual (SX1278)
    hal_delay(10);											//Hace un delay de 100us (exigido por el fabricante
    hal_pin_rst(1);											//Coloca el pin de RESET del modem como FLOTANTE (pin que antes era salida, ahora es entrada)
#endif
//    hal_delay(10);											//Hace un delay de 100us (exigido por el fabricante
//    hal_pin_rst(2);											//Coloca el pin de RESET del modem como FLOTANTE (pin que antes era salida, ahora es entrada)
    hal_delay(10);											//Hace un delay de 5ms recomendado por el fabricante
...
void radio_Checkpoint_Version()
{
	...
	versionCheckpoint = readReg(RegVersion);							//Pregunta por la version del hardware conectado ->12 SX1278  ->22 SX1272
    //Punto de checkpoint
	RADIO.version = versionCheckpoint;
	ASSERT(versionCheckpoint == versionLora);										//EN CASO DE NO ESTAR CONECTADO INDICA QUE NO ESTA CONECTADO EL LORA
}
...
/*
    //Parte de codigo del rado_init() desde la verificacion de la version
    //Comienza el "calentamiento" del modem con la lectura de RSSI
    //Generara 16 bytes para probar con RSSI (unicamente para testeo, los bytes generados nunca se usan)

    opmodeLora();											//Para ser configurado

    opmode(OPMODE_STANDBY);														//El modo STANDBY es requerido para la configuracion del LORA
        // don't use MAC settings at startup									//Los siguiente valores escritos son temporales
...
//#define RXLORA_RXMODE_RSSI_REG_MODEM_CONFIG3     0x04					//Low DatarateOptimize->0     AgcAutoOn->1  LNA gain set by internal AGC Loop*
    ...
    writeReg(LORARegModemConfig1, RXLORA_RXMODE_RSSI_REG_MODEM_CONFIG1);		//BW=125Khz     Code rating=4/5    Explicit header
    writeReg(LORARegModemConfig2, RXLORA_RXMODE_RSSI_REG_MODEM_CONFIG2);		//SF=7 -> 128chips/symbols   TxContinousMode=0-> Single Packet  RxPayloadCrcOn-> 0    SymbTimeout->00
    writeReg(LORARegModemConfig2, RXLORA_RXMODE_RSSI_REG_MODEM_CONFIG3);		//Low DatarateOptimize->0     AgcAutoOn->1  LNA gain set by internal AGC Loop*



    writeReg(RegLna, LNA_RX_GAIN);												//Aumento Max.    LNA max corriente 150%

    writeReg(LORARegPayloadMaxLength, 64);										//Determina que el maximo payload sera de 64 Bytes para TX o RX
    writeReg(LORARegSymbTimeoutLsb, 0);		 									//Escribe que el tiempo Timeout de simbolos es 0
    writeReg(LORARegSyncWord, LORA_MAC_PREAMBLE);								//Escribe la palabra reservada para LoRaWAN networks


    						//Configuracion de Pines DIO0=RXDONE DIO3=PAYLOADCRCERROR.
    						//Escribe el mapeo en RegDioMapping1{40}
    writeReg(RegDioMapping1, MAP_DIO0_LORA_RXDONE|MAP_DIO1_LORA_RXTOUT|MAP_DIO2_LORA_NOP|MAP_DIO3_PAYLOADCRCERROR);
    writeReg(LORARegIrqFlags, 0xFF);											//Limpia todas las banderas. Las banderas se limpian colocondolas en '1'

    writeReg(LORARegIrqFlagsMask, 0XFF);										//Deshabilita todas las interrupciones del LORA

    opmode(OPMODE_RX); 					//Empieza a recibir senales  RUIDO RSSI
    									//Esto solo lo hace para testear el modem. Los datos generados NO se usan
    for(int i=1; i<16; i++)
    {
        for(int j=0; j<8; j++)
        {
            ...
            randbuf[i] = (randbuf[i] << 1) | b;
    									...
    									//Hasta aqui testea el modem con ruido
